...
export function RevisionView({
  ...
  const revisionSubjects = useMemo(
    () =>
      data.subjects
        .map((subject) => ({
          subject,
          courses: coursesForSubject(subject.id, data.courses).filter(
            (course) => cardsForCourse(course.id, data.flashcards).length > 0,
          ),
        }))
        .filter((item) => item.courses.length > 0),
    [data.courses, data.flashcards, data.subjects],
  );
  ...
  const startSession = (difficultOnly: boolean, startedAt: number) => {
    ...
    const cards = data.flashcards.filter(
      (card) =>
        (selected.size === 0 || selected.has(card.courseId)) &&
        (!difficultOnly || progress.masteryByCard[card.id] === 'review'),
    );
  ...
  const rateCard = (status: MasteryStatus, ratedAt: number) => {
    ...
    const elapsed = Math.max(
      1,
      Math.min(300, Math.round((ratedAt - cardShownAt.current) / 1000)),
    );
    onProgressChange({
      ...progress,
  ...
  if (!data.flashcards.length) {
    return (
      <section className="revision-screen">
        <header className="section-header revision-header">
          <div>
            <p className="eyebrow">Mémorisation active</p>
            <h1>Réviser</h1>
            <p className="header-subtitle">
              Tes cartes apparaîtront ici automatiquement.
            </p>
          </div>
        ...
        <div className="revision-empty-state">
          ...
          <h2>Ajoute une photo de cours</h2>
          <p>
            Tempo transforme la photo en fiche claire, puis crée les questions
            et réponses nécessaires pour réviser.
          </p>
          <button onClick={onOpenCourses}>
            <Camera />
            <strong>Ajouter mon premier cours</strong>
            <ChevronRight />
          </button>
        </div>
      </section>
  ...
  if (sessionCards.length && !complete && currentCard) {
    const course = data.courses.find(
      (item) => item.id === currentCard.courseId,
    );
    const subject = data.subjects.find(
      (item) => item.id === currentCard.subjectId,
    );
    const percentage = Math.round((index / sessionCards.length) * 100);
    return (
      <section className="revision-screen active-session">
        <header className="learning-detail-header">
          <button className="back-button" onClick={resetSession}>
            <ArrowLeft /> <span>Quitter la session</span>
          </button>
        ...
        <Progress value={percentage} className="revision-progress" />
        <div className="flashcard-stage">
          ...
          >
            ...
            <span className="flip-hint">
              <RotateCcw /> Cliquer pour retourner
            </span>
          </button>
        </div>
        {flipped ? (
          <div className="rating-panel">
            <p>Tu connaissais la réponse ?</p>
            <div>
              {ratingOptions.map((option) => (
                <button
                  className={option.className}
                  key={option.status}
                  onClick={(event) => rateCard(option.status, event.timeStamp)}
                >
                  {option.status === 'review' ? (
                    <RotateCcw />
                  ) : option.status === 'medium' ? (
                    <Target />
                  ) : (
                    <Check />
                  )}
          ...
          <button className="reveal-button" onClick={() => setFlipped(true)}>
            Afficher la réponse <ChevronRight />
          </button>
        )}
      </section>
    );
  ...
  if (complete) {
    return (
      <section className="revision-screen revision-complete">
        <div className="complete-orb">
          <Sparkles />
        </div>
        <p className="eyebrow">Session terminée</p>
        <h1>
          {sessionCards.length ? 'Bien joué !' : 'Aucune carte à réviser'}
        </h1>
        <p>
          {sessionCards.length
            ? `${sessionCards.length} flashcards parcourues. Tes résultats sont enregistrés sur cet appareil.`
            : 'Sélectionne d’autres cours, ou révise les cartes difficiles après en avoir marqué “À revoir”.'}
        </p>
        <button onClick={resetSession}>Choisir une autre session</button>
      </section>
    );
  }
    ...
    <section className="revision-screen">
      <header className="section-header revision-header">
        <div>
          ...
          <h1>Réviser</h1>
          <p className="header-subtitle">
            Choisis tes cours, puis avance carte après carte.
          </p>
        </div>
        <button className="avatar" onClick={onOpenSettings}>
          CS
      ...
      <div className="revision-overview">
        ...
        <div>
          ...
          <p>
            {overall.mastered} acquises · {overall.review} à revoir
          </p>
        </div>
        <Progress
          value={overall.percentage}
          className="revision-overview-progress"
        />
      </div>
      <div className="revision-toolbar">
        <div>
          <h2>Composer une session</h2>
          <p>Aucune sélection = toutes les cartes.</p>
        </div>
        <button onClick={(event) => startSession(true, event.timeStamp)}>
          <Flame /> Revoir mes difficultés
        </button>
      </div>
      <div className="revision-subjects">
        {revisionSubjects.map(({ subject, courses: subjectCourses }) => {
          const subjectCourseIds = subjectCourses.map((course) => course.id);
          const allSelected = subjectCourseIds.every((id) =>
            ...
            <section key={subject.id}>
              <div className="revision-subject-title">
                ...
                <div>
                  <h3>{subject.name}</h3>
                  <p>
                    {subjectCourses.reduce(
                      (sum, course) =>
                        sum + cardsForCourse(course.id, data.flashcards).length,
                      0,
                    )}{' '}
                    cartes
                  </p>
                ...
                <button
                  type="button"
                  onClick={() => toggleSubject(subjectCourseIds)}
                >
                  {allSelected ? 'Retirer' : 'Tout choisir'}
                </button>
              </div>
              <div>
                {subjectCourses.map((course) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>
                        <strong>{course.title}</strong>
                        <small>{cards.length} flashcards</small>
                      </span>
                      <ChevronRight />
                    </button>
                  );
      ...
      <button
        className="start-revision"
        onClick={(event) => startSession(false, event.timeStamp)}
      >
        <TimerReset />
        <span>
          <strong>Lancer la révision</strong>
          <small>
            {selectedCourseIds.length
              ? `${selectedCourseIds.length} cours sélectionné${selectedCourseIds.length > 1 ? 's' : ''}`
              : `${data.flashcards.length} cartes disponibles`}
          </small>
        </span>
        <ChevronRight />
      </button>
