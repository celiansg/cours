...
const defaultSettings: AppSettings = {
  ...
  schoolStart: '2026-09-01', schoolEnd: '2027-06-30', calculationMode: 'calendar',
  excludedDates: ['2026-12-21', '2026-12-22', '2026-12-23', '2026-12-24', '2026-12-25'],
};
...
function calculateSchoolProgress(now: Date, settings: AppSettings, courses: Course[]) {
  ...
  const isActiveDay = (date: Date) => settings.calculationMode === 'calendar' || (date.getDay() >= 1 && date.getDay() <= 5 && !settings.excludedDates.includes(dateKey(date)) && courses.some(c => c.day === date.getDay() && c.type !== 'break'));
...
function earnedForDate(date: Date, now: Date, courses: Course[], settings: AppSettings, hourly: number) {
  ...
  const dayCourses = courses.filter(c => c.day === date.getDay() && c.type !== 'break'); const isToday = dateKey(date) === dateKey(now);
...
function YearCard({ now, settings, courses }: { now: Date; settings: AppSettings; courses: Course[] }) {
  ...
  return <section className="year-card" aria-labelledby="year-title">
    <div className="year-head"><div><p className="eyebrow" id="year-title">Année scolaire</p><h2>{school.start.getFullYear()} — {school.end.getFullYear()}</h2></div><div className="year-percentage"><strong>{school.percentage.toLocaleString('fr-FR', { maximumFractionDigits: 1 })}</strong><span>%</span></div></div>
...
function HomeScreen({ now, settings, courses, hourly, goTo }: { now: Date; settings: AppSettings; courses: Course[]; hourly: number; goTo: (tab: Tab) => void }) {
  const todayCourses = courses.filter(c => c.day === now.getDay()).sort((a, b) => a.start.localeCompare(b.start)); const minuteNow = now.getHours() * 60 + now.getMinutes() + now.getSeconds() / 60;
  ...
  return <>
    <header className="topbar"><div><p className="eyebrow">Aujourd’hui</p><h1>{longDate(now)}</h1></div><button className="avatar" onClick={() => goTo('settings')} aria-label="Ouvrir le profil">CS</button></header>
...
export default function Page() {
  ...
  useEffect(() => { try { const savedSettings = localStorage.getItem('tempo-settings'); const savedCourses = localStorage.getItem('tempo-courses'); if (savedSettings) setSettings(JSON.parse(savedSettings)); if (savedCourses) setCourses(JSON.parse(savedCourses)); } finally { setHydrated(true); } }, []);
