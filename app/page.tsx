...
import {
  ArrowUpRight, BarChart3, BookOpen, BrainCircuit, CalendarDays, Check, ChevronRight, Clock3, Coins,
  Edit3, Home, MapPin, Plus, Sparkles, Trash2, TrendingUp, WalletCards,
} from 'lucide-react';
...
import type { LearningData, LearningState, RevisionProgress } from '@/src/domain/learning';
...
function HomeScreen({ now, settings, courses, hourly, learning, goTo }: { now: Date; settings: AppSettings; courses: Course[]; hourly: number; learning: LearningState; goTo: (tab: Tab) => void }) {
  ...
  const workedToday = earnedForDate(now, now, courses, settings, hourly); const paidMinutesToday = todayCourses.filter(course => course.type !== 'break').reduce((sum, course) => sum + clamp(minuteNow - timeToMinutes(course.start), 0, timeToMinutes(course.end) - timeToMinutes(course.start)), 0); const earnedCurrent = current && current.type !== 'break' ? (minuteNow - timeToMinutes(current.start)) / 60 * hourly : 0;
  ...
  return <>
    <header className="topbar"><div><div className="brand-lockup"><span className="brand-logo"><img src="/tempo-logo.png" alt="Logo Tempo" /></span><span><strong>Tempo</strong><small>Cours & alternance</small></span></div><p className="eyebrow">Aujourd’hui</p><h1>{longDate(now)}</h1><span className={`day-context ${todayKind}`}>{todayKind === 'school' ? 'Cours · Lycée La Fayette' : todayKind === 'company' ? 'Entreprise' : todayKind === 'holiday' ? 'Jour sans cours' : 'Repos'}</span></div><button className="avatar" onClick={() => goTo('settings')} aria-label="Ouvrir le profil">CS</button></header>
    <section className="hero-grid" aria-label="Vue d’ensemble"><article className="glass-card course-card"><div className="card-heading"><div><span className={`status ${current ? 'live' : ''}`}><span />{current ? current.type === 'break' ? 'Pause en cours' : 'En cours' : next ? 'À venir' : todayCourses.length ? 'Terminé' : 'Repos'}</span><h2>{current?.name ?? next?.name ?? (todayCourses.length ? 'Journée terminée 🎉' : 'Aucun cours aujourd’hui')}</h2></div><div className="course-icon"><Sparkles size={21} /></div></div>{(current || next) && <div className="time-row"><span>{(current ?? next)?.start}</span><i /><span>{(current ?? next)?.end}</span></div>}<div className="course-metric"><strong>{Math.round(progress)}<small>%</small></strong><div><span>{current ? `${durationLabel(minuteNow - timeToMinutes(current.start))} écoulées` : next ? `Commence dans ${durationLabel(timeToMinutes(next.start) - minuteNow)}` : '100 % de la journée'}</span><b>{current ? `${durationLabel(timeToMinutes(current.end) - minuteNow)} restantes` : next ? `Prochain rendez-vous à ${next.start}` : todayCourses.length ? 'Bravo, à demain' : 'Reprise au prochain jour planifié'}</b></div></div><Progress value={progress} className="course-progress" /></article>
      <article className="glass-card income-card"><div className="income-top"><div className="income-icon"><WalletCards size={22} /></div><span className="soft-badge">+{money(hourly / 60)} €/min</span></div><p>Revenus en direct</p><strong>+ {money(earnedCurrent)} €</strong><span>Depuis le début du cours</span><div className="income-footer"><div><span>Équivalent aujourd’hui</span><small>{durationLabel(paidMinutesToday)} valorisées à {money(hourly)} €/h</small></div><b>+ {money(workedToday)} €</b></div></article></section>
    <YearCard now={now} settings={settings} courses={courses} />
    <section className="quick-grid"><article className="mini-card"><div><Clock3 size={19} /><span>Journée</span></div><strong>{Math.round(dayProgress)} %</strong><p>{last && minuteNow < timeToMinutes(last.end) ? `Fin dans ${durationLabel(timeToMinutes(last.end) - minuteNow)}` : todayCourses.length ? 'Terminée pour aujourd’hui' : 'Journée libre'}</p><Progress value={dayProgress} className="mini-progress" /></article><article className="mini-card next-card"><div><CalendarDays size={19} /><span>Prochaine étape</span></div><strong>{next?.name ?? (current ? `Fin de ${current.name}` : 'Planning à consulter')}</strong><p>{next ? `${next.start} → ${next.end}` : current ? current.end : 'Préparez la suite'}</p><button onClick={() => goTo('planning')}>Voir le planning <ArrowUpRight size={16} /></button></article><article className="mini-card revision-home-card"><div><BrainCircuit size={19} /><span>Révision</span></div><strong>{mastery.review} carte{mastery.review > 1 ? 's' : ''} à revoir</strong><p>{mastery.percentage} % de maîtrise globale</p><button onClick={() => goTo('review')}>Lancer une session <ArrowUpRight size={16} /></button></article></section>
  </>;
}

function PlanningScreen({ now, courses, setCourses, onOpenSettings }: { now: Date; courses: Course[]; setCourses: React.Dispatch<React.SetStateAction<Course[]>>; onOpenSettings: () => void }) {
...
function StatsScreen({ now, settings, courses, hourly, learning, onOpenSettings }: { now: Date; settings: AppSettings; courses: Course[]; hourly: number; learning: LearningState; onOpenSettings: () => void }) {
  ...
  const mastery = summarizeMastery(learning.data.flashcards, learning.progress); const revisionMinutes = Math.round(learning.progress.revisionSeconds / 60); const subjectMastery = learning.data.subjects.map(subject => ({ subject, summary: summarizeMastery(cardsForSubject(subject.id, learning.data.flashcards), learning.progress) })).sort((a, b) => a.summary.percentage - b.summary.percentage);
  return <section className="screen-section"><header className="section-header"><div><p className="eyebrow">Vue d’ensemble</p><h1>Statistiques</h1></div><div className="header-actions"><div className="header-chip"><TrendingUp size={17} /> En direct</div><button className="avatar small-avatar" onClick={onOpenSettings}>CS</button></div></header><article className="total-card"><div className="total-orb"><Coins size={26} /></div><p>Gagnés au total</p><strong>{money(total)} €</strong><span>Depuis le {shortDate(schoolStart)}</span></article><div className="stat-grid"><article><span>Par minute</span><strong>{money(hourly / 60)} €</strong></article><article><span>Par heure</span><strong>{money(hourly)} €</strong></article><article><span>Aujourd’hui</span><strong>{money(today)} €</strong></article><article><span>Cette semaine</span><strong>{money(week)} €</strong></article></div><article className="learning-stats-card"><div className="learning-stats-head"><div><p className="eyebrow">Révisions</p><h2>Maîtrise des cours</h2></div><strong>{mastery.percentage} %</strong></div><Progress value={mastery.percentage} className="learning-stat-progress" /><div className="learning-stat-metrics"><div><span>Cartes révisées</span><strong>{learning.progress.reviewedCount}</strong></div><div><span>Temps de révision</span><strong>{revisionMinutes} min</strong></div><div><span>À revoir</span><strong>{mastery.review}</strong></div></div><p className="subject-stat-caption">Matières les plus difficiles</p><div className="subject-stat-list">{subjectMastery.map(({ subject, summary }) => <div key={subject.id}><span style={{ background: subject.accent }} /><strong>{subject.name}</strong><Progress value={summary.percentage} className="subject-stat-progress" /><b>{summary.percentage} %</b></div>)}</div></article><article className="chart-card"><div className="chart-head"><div><p className="eyebrow">7 derniers jours</p><h2>Revenus estimés</h2></div><strong>{money(week)} €</strong></div><div className="bar-chart">{bars.map((bar, i) => <div className="bar-column" key={`${bar.label}-${i}`}><div className="bar-value">{bar.value ? Math.round(bar.value) : ''}</div><div className="bar-rail"><div style={{ height: `${Math.max(5, bar.value / max * 100)}%` }} /></div><span>{bar.label}</span></div>)}</div></article><div className="long-stats"><article><span>Ce mois</span><strong>{money(month)} €</strong><ChevronRight /></article><article><span>Depuis le début</span><strong>{money(total)} €</strong><ChevronRight /></article></div></section>;
}
...
const navItems: { id: Tab; label: string; icon: typeof Home }[] = [{ id: 'home', label: 'Accueil', icon: Home }, { id: 'planning', label: 'Planning', icon: CalendarDays }, { id: 'courses', label: 'Cours', icon: BookOpen }, { id: 'review', label: 'Réviser', icon: BrainCircuit }, { id: 'stats', label: 'Stats', icon: BarChart3 }];

export default function Page() {
  const [tab, setTab] = useState<Tab>('home'); const [now, setNow] = useState(() => new Date()); const [settings, setSettings] = useState(defaultSettings); const [courses, setCourses] = useState(defaultCourses); const [hydrated, setHydrated] = useState(false); const [learning, setLearning] = useState<LearningState>(() => loadLearningState()); const [revisionRequest, setRevisionRequest] = useState<string[]>([]);
  ...
  useEffect(() => { saveLearningState(learning); }, [learning]);
  ...
  const hourly = settings.manualRateEnabled ? settings.manualRate : settings.salary / (settings.hoursWeek * 52 / 12);
  const updateLearningData = (data: LearningData) => setLearning(current => ({ ...current, data })); const updateRevisionProgress = (progress: RevisionProgress) => setLearning(current => ({ ...current, progress })); const openSettings = () => setTab('settings');
  const startRevision = (courseIds: string[]) => { setRevisionRequest(courseIds); setTab('review'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  return <main className="app-shell"><div className="ambient ambient-one" /><div className="ambient ambient-two" /><div className="app-content">{tab === 'home' && <HomeScreen now={now} settings={settings} courses={courses} hourly={hourly} learning={learning} goTo={setTab} />}{tab === 'planning' && <PlanningScreen now={now} courses={courses} setCourses={setCourses} onOpenSettings={openSettings} />}{tab === 'courses' && <CoursesView data={learning.data} progress={learning.progress} onDataChange={updateLearningData} onStartRevision={startRevision} onOpenSettings={openSettings} />}{tab === 'review' && <RevisionView data={learning.data} progress={learning.progress} requestedCourseIds={revisionRequest} onProgressChange={updateRevisionProgress} onOpenSettings={openSettings} />}{tab === 'stats' && <StatsScreen now={now} settings={settings} courses={courses} hourly={hourly} learning={learning} onOpenSettings={openSettings} />}{tab === 'settings' && <SettingsScreen settings={settings} setSettings={setSettings} />}</div><nav className="bottom-nav" aria-label="Navigation principale">{navItems.map(({ id, label, icon: Icon }) => <button className={tab === id ? 'active' : ''} onClick={() => { setTab(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} key={id} aria-current={tab === id ? 'page' : undefined}><Icon size={20} strokeWidth={2.1} /><span>{label}</span></button>)}</nav></main>;
}
